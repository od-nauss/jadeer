...
export default async function EvaluationPage({
  ...
  const { data: link } = await supabase
    ...
    .select('*, approved_evaluators(full_name, email, relationship_type), candidate_profiles(users(full_name, job_title))')
  ...
  type LinkData = {
    approved_evaluators: { full_name: string; email: string; relationship_type: string };
        ...
        <div className="institutional-card p-6 mb-6">
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="font-bold text-primary-700">{data.approved_evaluators.full_name}</div>
              <div className="text-xs text-darkgray mt-0.5">{data.approved_evaluators.relationship_type}</div>
            </div>
