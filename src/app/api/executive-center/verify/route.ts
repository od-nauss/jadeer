...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.EXEC_CENTER_NO_PASSWORD === 'true' && process.env.NODE_ENV === 'development') {
      const cookieStore = cookies();
      cookieStore.set('executive_center_access', 'granted', { httpOnly: true, secure: false, sameSite: 'lax', maxAge: 60 * 60 * 24, path: '/' });
