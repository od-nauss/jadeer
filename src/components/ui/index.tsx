...
export function PageHeader({ title, description, example, icon: Icon, actions }: PageHeaderProps) {
  return (
    <div className="mb-8">
...
export function StatCard({ label, value, hint, variant = 'primary', icon: Icon }: StatCardProps) {
  const variants = {
  ...
  return (
    <div className={cn('rounded-xl border-2 p-5 transition-all hover:shadow-md', variants[variant])}>
      <div className="flex items-start justify-between mb-2">
        <span className="text-xs font-medium text-darkgray uppercase tracking-wide">
          {label}
        </span>
...
export function DemoDataNotice({ className }: DemoDataNoticeProps) {
  return (
    <div
      className={cn(
        'flex items-start gap-2 px-3 py-2 bg-gold-50 border border-gold-300 rounded-lg text-xs text-gold-800',
        className
      )}
    >
