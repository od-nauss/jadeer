...
const DEMO_CANDIDATES = [
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    card: {
      ...
      ai_summary: `سعد محمد كفاءة استراتيجية نادرة بالمنظومة الأمنية الأكاديمية. درجة ثقة في التصنيف: عالية (81%). يُوصى بمسار تأهيل لمنصب مدير عام خلال 12 شهراً، مشروطاً بإتمام برنامج القيادة التحويلية وتكليف إداري تجريبي. أبرز مخاطر التصنيف: رضا الفريق 68% يُشير إلى فجوة في الأسلوب القيادي اليومي رغم القوة الاستراتيجية العالية.`,
      is_published: true,
      is_demo: true,
    },
    initiatives: [
    ...
    evaluators: [
      { name: 'د. عبدالرحمن أحمد', relation: 'manager', email: 'alzahrani.demo@jadeer.sa', status: 'submitted' },
      { name: 'م. نوف سالم', relation: 'colleague', email: 'notaibi.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. سلطان راشد', relation: 'colleague', email: 'albaqmi.demo@jadeer.sa', status: 'submitted' },
      { name: 'د. مها سعيد', relation: 'subordinate', email: 'alsahli.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. فيصل ناصر', relation: 'internal_client', email: 'aldosari.demo@jadeer.sa', status: 'submitted' },
    ],
    ...
    govDecision: {
      ...
      reason: 'بعد مراجعة الملف القيادي والتقييم المتعدد المصادر، ترى لجنة الحوكمة أن المرشح سعد محمد يمتلك كفاءة استراتيجية عالية تؤهله للمناصب القيادية العليا. يُشترط إتمام برنامج القيادة التحويلية والتكليف التجريبي المقرر قبل صدور القرار النهائي بالترقية.',
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    card: {
      ...
      ai_summary: `نورة عبدالله نموذج القائد التشغيلي الجاهز فوراً. درجة ثقة عالية جداً (89%). يُوصى بتعيينها مديرة إدارة العمليات في أقرب فرصة ممكنة. أداؤها الموثق يتجاوز متطلبات المنصب بمرحلتين. الفجوة الوحيدة (التفكير الاستراتيجي 74%) لا تُعيق القرار الفوري وتُعالج بسهولة خلال السنة الأولى في المنصب.`,
      is_published: true,
      is_demo: true,
    },
    initiatives: [
    ...
    evaluators: [
      { name: 'م. عبدالكريم علي', relation: 'manager', email: 'alshamri.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. ريم وليد', relation: 'colleague', email: 'alharbi.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. تركي سعد', relation: 'subordinate', email: 'alanazi.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. منال السلمي', relation: 'subordinate', email: 'alsalmi.demo@jadeer.sa', status: 'submitted' },
      { name: 'م. بندر أحمد', relation: 'internal_client', email: 'bzahrani.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. لمياء خالد', relation: 'colleague', email: 'alasiri.demo@jadeer.sa', status: 'submitted' },
    ],
    ...
    govDecision: {
      ...
      reason: 'بعد مراجعة معمّقة لملف المرشحة نورة عبدالله، يُقرر مجلس الحوكمة الاعتماد الكامل بالإجماع. تمتلك المرشحة سجلاً تشغيلياً استثنائياً موثقاً بمؤشرات قابلة للقياس، وتقييم 360° ممتاز يعكس ثقة الفريق والمستفيدين. تُوصي اللجنة بالترقية الفورية لمنصب مديرة إدارة العمليات.',
      conditions: [],
    },
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    evaluators: [
      ...
      { name: 'م. سارة علي', relation: 'colleague', email: 'alghamdi.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. وليد راشد', relation: 'subordinate', email: 'albalawi.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. حنان عمر', relation: 'internal_client', email: 'almalki.demo@jadeer.sa', status: 'submitted' },
    ],
    ...
    govDecision: {
      ...
      reason: 'يُقرّ مجلس الحوكمة أن عبدالعزيز سالم يمتلك كفاءة تقنية استثنائية مؤهِّلة للقيادة التقنية المستقبلية. غير أن درجات القيادة الإنسانية ورضا الفريق تستلزم مسار تطوير منهجي قبل التكليف القيادي. تُعتمد خطة التطوير المقترحة ويُعاد تقييم الملف خلال 18 شهراً.',
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    evaluators: [
      ...
      { name: 'أ. بدر خالد', relation: 'colleague', email: 'alshahri.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. شيماء خالد', relation: 'subordinate', email: 'shmutairi.demo@jadeer.sa', status: 'submitted' },
    ...
    govDecision: {
      decision_type: 'conditional_approval',
      reason: 'تمتلك هند عمر موهبة قيادية إنسانية نادرة تحتاجها المنظمة في مناصب دعم الفرق. تعتمد اللجنة التعيين في رئاسة قسم دعم الفرق، مشروطاً بإتمام برنامج قياس الأثر خلال 6 أشهر وتقديم داشبورد مؤشرات لمجلس الإدارة.',
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    card: {
      ...
      ai_summary: `فهد خالد نموذج نادر للمُنجز الفذّ الذي يعاني أزمة قيادية جوهرية. رضا الفريق 31% مع دوران وظيفي 28% يُنذر بأزمة مؤسسية خطيرة إذا رُقِّي للمناصب القيادية الأعلى. نتائج تقييم 360 كشفت أن 4 من 6 مقيمين يصفون أسلوبه بـ"الضغط الإداري المُرهق" رغم إقرارهم بكفاءته. يُوصى بالإبقاء كخبير امتثال أول مع برنامج علاج قيادي صارم، وعدم الترقية للمناصب الإشرافية حتى يُظهر تحسناً قابلاً للقياس في رضا الفريق.`,
      is_published: true,
      is_demo: true,
    },
    initiatives: [
    ...
    evaluators: [
      { name: 'أ. صالح ناصر', relation: 'manager', email: 'sasiri.demo@jadeer.sa', status: 'submitted' },
      { name: 'أ. جواهر سالم', relation: 'colleague', email: 'alqarni.demo@jadeer.sa', status: 'submitted' },
      ...
      { name: 'أ. حمد سعد', relation: 'subordinate', email: 'hghamdi.demo@jadeer.sa', status: 'submitted' },
    ...
    devPlan: {
      ...
      items: [
        ...
        { title: 'جلسات استماع مع أعضاء الفريق', category: 'exposure', description: 'جلسات استماع فردية شهرية مع كل عضو في فريقه لفهم احتياجاتهم', due_date: '2025-12-31', status: 'in_progress', responsible: 'هند عمر كوسيطة' },
    ...
    govDecision: {
      ...
      reason: 'يُوقف مجلس الحوكمة النظر في أي ترقية لفهد خالد لمدة 12 شهراً، استناداً إلى المؤشرات الموضوعية التالية: معدل دوران الموظفين 28%، درجة رضا الفريق 31%، و7 شكاوى رسمية في عام واحد. يُقدَّر أداؤه الفردي الاستثنائي، ويُوصى بالاستثمار فيه كخبير امتثال أول مع برنامج تحول قيادي إلزامي، وإعادة العرض بعد اثني عشر شهراً مع تقرير موثق من المرشد القيادي.',
